...
var ref$, id, log, initial, tail, zip, tuples, Q, matmul, matadd, sigmoid, matrix, stringifyMatrix, randSd, GUI, net, layerSizes, weightShapes, weights, w, biases, h, activation, predict;
ref$ = require('utils'), id = ref$.id, log = ref$.log, initial = ref$.initial, tail = ref$.tail, zip = ref$.zip, tuples = ref$.tuples, Q = ref$.Q;
ref$ = require('math'), matmul = ref$.matmul, matadd = ref$.matadd, sigmoid = ref$.sigmoid, matrix = ref$.matrix, stringifyMatrix = ref$.stringifyMatrix, randSd = ref$.randSd;
GUI = require('dat.gui');
net = (layerSizes = [3, 5, 10], weightShapes = tuples(initial(layerSizes), tail(layerSizes)), weights = (function(){
  ...
  for (i$ = 0, len$ = (ref$ = weights).length; i$ < len$; ++i$) {
    ...
    w = ref$[i$];
    b = biases[ix];
    a = matadd(matmul(w, a), b).map(activation);
    log(a);
    log("Net::predict - layer:", stringifyMatrix(a));
  }
...
},{"dat.gui":1,"math":3,"utils":4}],3:[function(require,module,exports){
// Generated by LiveScript 1.6.0
var ref$, log, unwords, unlines, sqrt, ln, exp, sin, cos, random, PI, TAU, rand, randSd, randRange, sigmoid, matrix, stringifyMatrix, matmul, matadd, out$ = typeof exports != 'undefined' && exports || this;
ref$ = require('utils'), log = ref$.log, unwords = ref$.unwords, unlines = ref$.unlines;
sqrt = Math.sqrt, ln = Math.log, exp = Math.exp, sin = Math.sin, cos = Math.cos, random = Math.random, PI = Math.PI;
out$.TAU = TAU = PI * 2;
