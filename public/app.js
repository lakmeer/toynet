...
var ref$, id, log, initial, tail, zip, tuples, Q, add, sub, dot, sum, matmul, matadd, sigmoid, relu, matrix, stringifyMatrix, randSd, GUI, Plot, net, layerSizes, weightShapes, weights, w, biases, h, activation, predict;
...
GUI = require('dat.gui');
Plot = require('./plot');
net = (layerSizes = [3, 5, 10], weightShapes = tuples(initial(layerSizes), tail(layerSizes)), weights = (function(){
  var i$, ref$, len$, results$ = [];
  for (i$ = 0, len$ = (ref$ = weightShapes).length; i$ < len$; ++i$) {
    w = ref$[i$];
    results$.push(matrix(w, randSd));
  }
  return results$;
}()), biases = (function(){
  var i$, ref$, len$, results$ = [];
  for (i$ = 0, len$ = (ref$ = tail(layerSizes)).length; i$ < len$; ++i$) {
    h = ref$[i$];
    results$.push(matrix([h, 1]));
  }
  return results$;
}()), activation = sigmoid, predict = function(a){
  var i$, ref$, len$, ix, w, b;
  log("Net::predict - inputs:", stringifyMatrix(a));
  for (i$ = 0, len$ = (ref$ = weights).length; i$ < len$; ++i$) {
    ix = i$;
    w = ref$[i$];
    b = biases[ix];
    log('weights', stringifyMatrix(w));
    log('biases', stringifyMatrix(b));
    a = matadd(matmul(w, a), b);
    log("Net::predict - layer:", a);
  }
  return log("Net::predict - prediction:", stringifyMatrix(a));
}, {
  predict: predict
});
